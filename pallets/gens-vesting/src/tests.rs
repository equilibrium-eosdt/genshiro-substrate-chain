...
use gens_balances::{currency, BalanceGetter, BalanceSetter, SignedBalance};
use gens_utils::fx64;
...
fn vested_transfer_amount_low() {
    new_test_ext().execute_with(|| {
        ...
        let account_id = 1;
        set_pos_balance_with_agg_unsafe(&account_id, &currency::Currency::Gens, fx64!(100, 0));
        assert_err!(
        ...
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(&1, &currency::Currency::Gens),
            gens_balances::SignedBalance::Positive(fx64!(100, 0).into_inner() as u64)
        );
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(
                &module_account_id,
                &currency::Currency::Gens
            ),
            gens_balances::SignedBalance::Positive(fx64!(0, 0).into_inner() as u64)
        );
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(&2, &currency::Currency::Gens),
            gens_balances::SignedBalance::Positive(0)
        );
    });
}
...
fn vested_transfer_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(&1, &currency::Currency::Gens),
            gens_balances::SignedBalance::Positive(fx64!(90, 0).into_inner() as u64)
        );
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(
                &module_account_id,
            ...
            gens_balances::SignedBalance::Positive(fx64!(10, 0).into_inner() as u64)
        );
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(&2, &currency::Currency::Gens),
            gens_balances::SignedBalance::Positive(0)
...
fn vest_temp() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            gens_balances::SignedBalance::Positive(fx64!(9, 0).into_inner() as u64)
        );
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(&2, &currency::Currency::Gens),
            gens_balances::SignedBalance::Positive(fx64!(1, 0).into_inner() as u64)
        );
    });
}
...
fn vest_temp2() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            gens_balances::SignedBalance::Positive(fx64!(7, 0).into_inner() as u64)
        );
        assert_eq!(
            <ModuleBalances as BalanceGetter<u64, u64>>::get_balance(&2, &currency::Currency::Gens),
            gens_balances::SignedBalance::Positive(fx64!(3, 0).into_inner() as u64)
        );
    });
}
