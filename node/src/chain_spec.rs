use gens_node_runtime::{
    gens_primitives, opaque::SessionKeys, AccountId, AuraConfig, BalancesConfig,
...
fn testnet_genesis(
    ...
    GenesisConfig {
        ...
        gens_balances: Some(BalancesConfig {
            balances: endowed_accounts
                .iter()
                .cloned()
                .map(|k| (k, 1 << 50, gens_primitives::currency::Currency::Gens.value()))
                .collect(),
        }),
        ...
        gens_session_manager: Some(gens_session_manager::GenesisConfig {
            validators: initial_authorities
                .iter()
                .map(|(x, ..)| x.clone())
                .collect(),
        }),
        gens_vesting: Some(EqVestingConfig { vesting: vec![] }),
    }
}
