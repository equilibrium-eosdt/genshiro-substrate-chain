...
use crate::runtime;
use crate::runtime::{Balance, GensRuntime};
    ...
    for success in successes {
        ...
        } = success
        {
            ...
            for currency in Currency::iterator_with_usd() {
                if currency != &Currency::Gens {
                    empty_balances.insert(*currency, SignedBalance::Positive(0));
                }
            }
            empty_balances.insert(Currency::Gens, SignedBalance::Positive(treasury_ratio));

            let trsry_acc_id: runtime::AccountId = ModuleId(*b"ge/trsry").into_account();
            state
                .balances
                .entry(trsry_acc_id)
                .and_modify(|bal| {
