...
pub struct AccountStore<T: Balances> {
    #[store(returns = gens_balances::SignedBalance<<T as Balances>::Balance>)]
    pub account_id: T::AccountId,
    pub currency: gens_balances::currency::Currency,
}

#[derive(Clone, Debug, Eq, PartialEq, Store, Encode)]
pub struct BalancesAggregatesStore<T: Balances> {
    #[store(returns = gens_balances::BalancesAggregate<<T as Balances>::Balance>)]
    pub currency: gens_balances::currency::Currency,
