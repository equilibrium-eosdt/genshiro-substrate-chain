...
pub struct VestingStore<T: EqVesting> {
    #[store(returns = Option<gens_vesting::VestingInfo<<T as EqVesting>::Balance, T::BlockNumber>>)]
...
pub struct VestedTransferCall<T: EqVesting> {
    pub account_id: T::AccountId,
    pub schedule: gens_vesting::VestingInfo<<T as EqVesting>::Balance, T::BlockNumber>,
}
