...
pub struct Requester {
    pub client: Client<super::GensRuntime>,
    nonces: HashMap<super::runtime::AccountId, u32>,
    pub sudo: String,
}

impl Requester {
    pub fn new(client: Client<GensRuntime>) -> Self {
        Requester {
            client,
            nonces: HashMap::new(),
            sudo: "".to_string(),
    ...
    pub async fn with_sudo<C: Call<GensRuntime>>(
        &mut self,
        call: C,
...
pub type ChainCallSuccess = ExtrinsicSuccessWithFee<GensRuntime, (AccountKey, Balance)>;
pub type ChainCallResult = Result<Vec<ChainCallSuccess>, Box<dyn std::error::Error>>;

pub async fn call_chain<C: Call<GensRuntime> + Send + Sync>(
    ...
    let decoder = GensRuntime::create_decoder(client.metadata().clone());
    let success = client
...
pub async fn sudo_call_chain<C: Call<GensRuntime> + Send + Sync>(
    client: &Client<GensRuntime>,
    nonces: Arc<Mutex<DevNonces>>,
    call: C,
    sudo: &String,
...
pub async fn call_chain_unsigned<C: Call<GensRuntime> + Send + Sync>(
    client: &Client<GensRuntime>,
    call: C,
) -> Result<Vec<ChainCallSuccess>, Box<dyn std::error::Error>> {
        ...
        if let Some(fee_value) = fee {
            {
                ...
                locked_storage
                    ...
                    .and_modify(|bal| {
                        bal
                            .entry(Currency::Gens)
                            .and_modify(|genses| *genses = genses.sub_balance(*fee_value).unwrap());
                    } );
                ...
                locked_storage
                    .BalancesAggregates
                    .entry(Currency::Gens)
                    .and_modify(|gens| {gens.total_issuance = old_total_inssuance - fee_value;});
            }
