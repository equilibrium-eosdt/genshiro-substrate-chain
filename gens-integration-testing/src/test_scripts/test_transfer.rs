...
use crate::{assert_gens_test_data_keystore, IntegrationTestConfig};
use gens_balances::{currency::Currency, SignedBalance};
use gens_integration_testing_macro::tuple_to_vec;
use gens_utils::fx64;
...
pub async fn test_transfer(
    ...
    println!("Setting charlie's Gens balance");
    set_balance(
        client,
        nonces.clone(),
        charlie_acc.into(),
    ...
    charlie_balances.insert(Currency::Gens, SignedBalance::Positive(min_eq));
    state
        .balances
    ...
    dave_balances.insert(Currency::Gens, SignedBalance::Positive(min_eq));
