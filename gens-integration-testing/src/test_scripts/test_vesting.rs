...
use crate::runtime::{AccountId, GensRuntime};
...
use crate::{assert_gens_test_data, assert_gens_test_data_keystore, IntegrationTestConfig};
use core::marker::PhantomData;
use gens_balances::currency::Currency;
use gens_balances::SignedBalance;
...
pub async fn test_vesting(
    ...
    let zero_balances: HashMap<Currency, SignedBalance<u64>> = [
        (Currency::Gens, SignedBalance::Positive(0)),
    ...
    let shedule = gens_vesting::VestingInfo {
    ...
    bob_balance.insert(Currency::Gens, bob_sign_balance.clone());
    state
        .balances
    ...
    let vestn_acc_id: AccountId = ModuleId(*b"ge/vestn").into_account();
    ...
    }
    {
        ...
        assert_gens_test_data_keystore!(&state, &actual_state, &store);
    }
