...
use codec::Encode;
pub use gens_balances;
pub use gens_distribution;
pub use gens_primitives;
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("Genshiro"),
    impl_name: create_runtime_str!("Genshiro"),
...
impl pallet_session::Trait for Runtime {
    ...
    type SessionManager = gens_session_manager::Module<Runtime>;
...
impl system::Trait for Runtime {
    ...
    type OnNewAccount = ();
    type OnKilledAccount = (gens_balances::Module<Runtime>);
    type AccountData = ();
...
parameter_types! {
    pub const TreasuryModuleId: ModuleId = ModuleId(*b"ge/trsry");
    pub const ParachainOfferingModuleId: ModuleId = ModuleId(*b"ge/prcho");
...
parameter_types! {
    ...
    pub const BasicCurrencyGet: gens_primitives::currency::Currency = gens_primitives::currency::Currency::Gens;
}

pub struct BalanceChecker;
type SignedBalance = gens_balances::SignedBalance<Balance>;

impl gens_balances::BalanceChecker<Balance, AccountId> for BalanceChecker {
    fn can_change_balance(
        who: &AccountId,
        currency: &gens_primitives::currency::Currency,
        ...
        let res = match change {
            ...
            SignedBalance::Negative(change_value) => {
                let balance =
                    <Balances as gens_balances::BalanceGetter<AccountId, Balance>>::get_balance(
                        who, currency,
                    );
                match balance {
...
impl gens_balances::Trait for Runtime {
    /// The type for recording an account's balance.
    type Balance = Balance;
    /// The ubiquitous event type.
    type Event = Event;

    type BalanceGetter = gens_balances::Module<Runtime>;
...
pub type BasicCurrency = gens_balances::balance_adapter::BalanceAdapter<
    Runtime,
    gens_balances::Module<Runtime>,
    BasicCurrencyGet,
>;
...
type EqImbalance = gens_balances::NegativeImbalance<Balance>;
...
impl frame_support::traits::OnUnbalanced<EqImbalance> for DealWithFees {
    fn on_unbalanceds<B>(mut fees_then_tips: impl Iterator<Item = EqImbalance>) {
        if let Some(fees) = fees_then_tips.next() {
            ...
            <Balances as gens_balances::GensCurrency<AccountId, Balance>>::resolve_creating(
                gens_balances::currency::Currency::Gens,
                &Authorship::author(),
                split.1,
            );
...
parameter_types! {
    pub const MinVestedTransfer: Balance = 1_000_000_000; // 1 gens
    pub const VestingModuleId: ModuleId = ModuleId(*b"ge/vestn");
    pub Prefix: &'static [u8] = b"Pay TEST EQ to the TEST account:";
}

impl gens_vesting::Trait for Runtime {
    type Event = Event;
    type Currency = BasicCurrency;
...
type Treasury = gens_distribution::Instance1;
impl gens_distribution::Trait<Treasury> for Runtime {
    type ModuleId = TreasuryModuleId;
}

type ParachainOffering = gens_distribution::Instance2;
impl gens_distribution::Trait<ParachainOffering> for Runtime {
    type ModuleId = ParachainOfferingModuleId;
}

impl gens_session_manager::Trait for Runtime {
    type Event = Event;
    type ValidatorId = <Self as system::Trait>::AccountId;
...
construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic
    {
        ...
        Balances: gens_balances::{Module, Call, Storage, Config<T>, Event<T>},

        TransactionPayment: transaction_payment::{Module, Storage},
        Sudo: sudo::{Module, Call, Config<T>, Storage, Event<T>},
        GensSessionManager: gens_session_manager::{Module, Call, Storage, Event<T>, Config<T>,},
        EqTreasury: gens_distribution::<Instance1>::{Module, Storage},
        EqParachainOffering: gens_distribution::<Instance2>::{Module, Storage},
        EqVesting: gens_vesting::{Module, Call, Storage, Event<T>, Config<T>},
    }
);
...
impl_runtime_apis! {
    ...
    impl frame_benchmarking::Benchmark<Block> for Runtime {
        fn dispatch_benchmark(
            ...
            add_benchmark!(params, batches, gens_balances, Balances);
            add_benchmark!(params, batches, gens_vesting, EqVesting);
